...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
     ...
     if (eSwId == MSG22XX_SW_ID_HLT)
        ...
        else if (eSwId == MSG22XX_SW_ID_SY)
        {
        ...
        if (nUpdateBinMinor > nMinor)
        {
         ...
         else   if (eSwId == MSG22XX_SW_ID_JL)
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_jl_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_jl_update_bin[i*1024]), 512);
                    }
                }
            }
            else if (eSwId == MSG22XX_SW_ID_SY)
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_sy_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_sy_update_bin[i*1024]), 512);
                    }
                }
            }
#endif
...
//    DBG(&g_I2cClient->dev, "*** %s() *** mutex_lock(&g_Mutex)\n", __func__);  // add for debug
    ...
    {
        DBG(&g_I2cClient->dev, "Set gesture debug mode packet length, g_ChipType=%d\n", g_ChipType);
    ...
    {
        DBG(&g_I2cClient->dev, "Set gesture wakeup packet length, g_ChipType=%d\n", g_ChipType);
